...
var HugoPublishSettingTab = class extends import_obsidian.PluginSettingTab {
  ...
  display() {
    ...
    new import_obsidian.Setting(containerEl).setName("exclude dir").setDesc('Exclude dir when syncing, relative path to vault, split by ","').addText((text4) => text4.setPlaceholder("templates,dir2,tmp/dir3").setValue(this.plugin.settings.exclude_dir).onChange(async (value) => {
...
var get_md_yaml_header_from_content = (content3) => {
...
var transform_better_latex = (ast) => {
  visit(ast, "math", function(node2, index2, parent) {
    const new_value = "<div>\n$$\n" + node2.value + "\n$$\n</div>";
...
var HugoPublishPlugin = class extends import_obsidian2.Plugin {
  ...
  async sync_blog() {
    ...
    for (let i = 0; i < blogs.length; i++) {
      ...
      let [header, body] = get_md_yaml_header_from_content(content3);
      ...
      if (hv) {
        ...
        if (!this.settings.export_blog_tag && this.settings.blog_tag.length > 0 && "tags" in hv) {
          hv["tags"] = hv["tags"].filter((v) => v !== this.settings.blog_tag);
        }
      }
